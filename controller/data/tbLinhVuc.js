...
import { getIsActiveItem, getDanhSachPhanCapLinhVuc, checkIsExisted, updateMulti } from '../../common/utils/filter';
...
async function getTable(req, res) {
  let rhApiUrl = _configs.rh.dataUrl + '/tbLinhVuc' + await getDanhSachPhanCapLinhVuc(req);;
...
async function post(req, res) {
  ...
  let checkIsEx = await checkIsExisted('tbLinhVuc', 'Ma', req.body.Ma)
  if (checkIsEx) {
  ...
  let rhApiUrl = _configs.rh.dataUrl + '/tbLinhVuc' + req.url;
...
function patch(req, res) {
  ...
  utils.Axios('patch', rhApiUrl, req.body)//method, url, data
    .then(async function (rhApiRes) {
      //update multi
      await updateMulti('tbDanhMuc', { 'LinhVuc.code': body.code }, { 'LinhVuc': body, DonViCha: body.DonViCha })
      //hết update multi
      mwLog.generate(req, { status: rhApiRes.status, body: rhApiRes.data });
      res.status(rhApiRes.status).send(rhApiRes.data);
    }).catch(function (rhApiErr) {
