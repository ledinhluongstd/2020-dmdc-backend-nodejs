...
import { getIsActiveItem, updateMulti } from '../../common/utils/filter';
...
function getTable(req, res) {
  let rhApiUrl = _configs.rh.dataUrl + '/tbNhomQuyen' + getIsActiveItem(req);;
...
function post(req, res) {
  ...
  let rhApiUrl = _configs.rh.dataUrl + '/tbNhomQuyen' + req.url;
...
function patch(req, res) {
  ...
  utils.Axios('patch', rhApiUrl, req.body)//method, url, data
    .then(async function (rhApiRes) {
      //update multi
      await updateMulti('tbNhomQuyenNguoiDung', { 'NhomQuyen.code': body.code }, { 'NhomQuyen': body })
      //hết update multi
      mwLog.generate(req, { status: rhApiRes.status, body: rhApiRes.data });
      res.status(rhApiRes.status).send(rhApiRes.data);
    }).catch(function (rhApiErr) {
